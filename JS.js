...
// Add mousedown event listener on window to handle clicks outside the menu
  ...
  setFocusToMenuitem(newMenuitem) {
    // Fix: Added logic to update the roving tabindex for all menu items
    this.menuitemNodes.forEach(function (item) {
      if (item === newMenuitem) {
        item.tabIndex = 0; // Fix: Make the currently focused item tabbable
      } else {
        item.tabIndex = -1; // Fix: Make other items not tabbable
      }
    });
    // Fix: Set focus to the new menu item
    newMenuitem.focus();
  }
